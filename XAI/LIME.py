...
def main():
    ...
    print("Initialize explainer")
    explainer1 = TabularExplainer(
        explainers=["lime"],                         # The explainers to apply
        mode="regression",                             # The task type
        data=tabular_data1,                                   # The data for initializing the explainers
        model=combine1,                                       # The ML model to explain
        preprocess=lambda z: torch.FloatTensor(transformer1.transform(z))    # Converts raw features into the model input                                                  # Additional parameters
    )
    
    # Generate explanations
    test_instance1 = tabular_data1[:1]
    local_explanations1 = explainer1.explain(test_instance1)
    local_explanations1 = local_explanations1['lime']
    res = local_explanations1.get_explanations(index=0)
    print(res)
