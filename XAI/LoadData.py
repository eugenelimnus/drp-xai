...
def prep_data(args):
    ...
    big_df = genes_1.merge(genes_2, how='outer')
    ...
    if drug == "Erlotinib":
        ...
        gene_names = CTRP_exprs_drug.columns
        merge_df = pd.DataFrame(gene_names, columns=['ensg_v99'])
        merge_df = merge_df.merge(big_df,how='left', on='ensg_v99')
        gene_names_appr = merge_df['gene_name_v99'].values
        ...
        return X_tr, Y_tr, gCSI_exprs_drug.values, gCSI_aac_drug.values, GSE33072.values, GSE33072R, PDX_Erlotinib.values, PDX_ErloR, X_U, gene_names_appr
